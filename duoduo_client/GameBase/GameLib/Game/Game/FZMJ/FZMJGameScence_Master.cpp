...
void FZMJGameScence::updateUserRight()
{
	// 控制按钮 536870912
	if (!WidgetFun::getChildWidget(this,"Button_Master"))
	{
		utility::log("FZMJGameScence::updateUserRight()");
		return;
	}
...
void FZMJGameScence::initMaster()
{
	...
	for (int i=0;i<MAX_INDEX;i++)
	{
		...
		BYTE iCard = FZMJ::CGameLogic::Instance().SwitchToCardData(i);
...
void FZMJGameScence::defaultMaster(bool bRestZhaNiao)
{
	if (WidgetFun::getChildWidget(this,"GameMaterPlane"))
	{
		if (bRestZhaNiao)
...
void FZMJGameScence::HNMJButton_Master(cocos2d::Ref*,WidgetUserInfo*)
{
	if (!IsInGame())
	{
		return;
...
void FZMJGameScence::Button_MasterClose(cocos2d::Ref*,WidgetUserInfo*)
{
	if (!IsInGame())
	{
		return;
...
void FZMJGameScence::OnMasterHandCard( const void *pBuffer, WORD wDataSize )
{
	if (!IsInGame())
	{
		return;
...
void FZMJGameScence::OnMasterLeftCard( const void *pBuffer, WORD wDataSize )
{
	if (!IsInGame())
	{
		return;
...
void FZMJGameScence::NHMJ_MASTER_LEFTCARD(cocos2d::Ref*,WidgetUserInfo* pUseInfo)
{
	if (!IsInGame())
	{
		return;
...
void FZMJGameScence::MasterZhaNiao(cocos2d::Ref*,WidgetUserInfo* pUserInfo)
{
...
void FZMJGameScence::setMasterCheakCard(BYTE cbCard)
{
