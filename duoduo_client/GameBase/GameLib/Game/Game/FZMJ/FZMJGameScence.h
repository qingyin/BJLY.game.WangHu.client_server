...
class FZMJGameScence
	:public GameBase
	,public TimeNode
	,public FvSingleton<FZMJGameScence> 
{
public:
	const static int KIND_ID = 321;
...
public:
	FZMJGameScence();
	FZMJGameScence(int iGameType,int iOption);
	~FZMJGameScence();
public:
	virtual bool init();
...
public:
	FZMJPlayer* getPlayerByChairID(int iChairID);
...
protected:
	...
	FZMJPlayer*					m_pLocal;
	FZMJPlayer*					m_pPlayer[MAX_PLAYER];
