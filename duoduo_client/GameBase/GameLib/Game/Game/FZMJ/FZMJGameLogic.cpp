...
FV_SINGLETON_STORAGE(FZMJ::CGameLogic);
namespace FZMJ
{
	//静态变量
	...
	const BYTE CGameLogic::m_cbCardDataArray[MAX_REPERTORY]=
	{
		...
		0x31,0x32,0x33,0x34,0x35,0x36,0x37,							        //字牌
		0x31,0x32,0x33,0x34,0x35,0x36,0x37,							        //字牌
		0x31,0x32,0x33,0x34,0x35,0x36,0x37,							        //字牌
		0x31,0x32,0x33,0x34,0x35,0x36,0x37,							        //字牌
	};
	...
	*/
	...
	bool CGameLogic::AnalyseCard(const BYTE cbCardIndex[MAX_INDEX], const CMD_WeaveItem WeaveItem[], BYTE cbWeaveCount, AnalyseItemList & kAnalyseItemList)
	{
		...
		return ((int)kAnalyseItemList.size()>0);
	}
	...
	{
		kTingCardList.clear();
		FZMJ::AnalyseItemList kAnalyseItemList;

		FZMJ::CGameLogic::Instance().SetMagicIndex(SwitchToCardIndex(0x35));
		//构造扑克
		BYTE cbCardIndexTemp[MAX_INDEX];
		memcpy(cbCardIndexTemp,cbCardIndex,sizeof(cbCardIndexTemp));
		...
		for (int i=0;i<MAX_INDEX-ZI_PAI_COUNT;i++)
		{
			...
			bool bValue = FZMJ::CGameLogic::Instance().AnalyseCard(cbCardIndexTemp,WeaveItem,cbWeaveCount,kAnalyseItemList);
			if (bValue)
			{
				kTingCardList.push_back(FZMJ::CGameLogic::Instance().SwitchToCardData(i));
			}
			cbCardIndexTemp[i]--;
		}
