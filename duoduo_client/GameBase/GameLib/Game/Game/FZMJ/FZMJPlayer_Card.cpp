...
void FZMJPlayer::setHandCard(BYTE* pCardData,int iCout)
{
	memcpy(m_kHandCardData,pCardData,sizeof(m_kHandCardData));
	m_kHandCardCout = iCout;
	FZMJ::CGameLogic::Instance().SortCardList(m_kHandCardData,m_kHandCardCout);
}
void FZMJPlayer::setWeaveItem(CMD_WeaveItem* pWeave,int iCout)
{
	memcpy(m_kWeaveItemArray,pWeave,sizeof(m_kWeaveItemArray));
	m_kWeaveCount = iCout;
}
void FZMJPlayer::setOperateResoult(CMD_S_OperateResult* pNetInfo)
{
	...
	if (pNetInfo->cbOperateCode == WIK_LEFT||pNetInfo->cbOperateCode == WIK_CENTER||pNetInfo->cbOperateCode == WIK_RIGHT)
	{
		...
		|| pNetInfo->cbOperateCode == WIK_CS_GANG
		||pNetInfo->cbOperateCode == WIK_CS_GANG)
...
//		if (m_iIdex == 0 || m_bGameRecord|| m_bMingPai )
		{
			 FZMJ::CGameLogic::Instance().RemoveValueCardAll(m_kHandCardData,m_kHandCardCout,pNetInfo->cbOperateCard);
		}
...
//		if (m_iIdex == 0 || m_bGameRecord || m_bMingPai)
		{
			...
			if (pNetInfo->cbOperateCode == WIK_RIGHT)
			{
			 ...
			 FZMJ::CGameLogic::Instance().RemoveValueCard(m_kHandCardData,m_kHandCardCout,cbWeaveCard,2);
//			m_kHandCardCout -= 2;
		}
//		else
		{
...
void FZMJPlayer::addNewInCard(int iCard)
{
	m_kHandCardCout++;
	m_kHandCardData[MAX_COUNT-1-m_kWeaveCount*3] = iCard;
}
BYTE FZMJPlayer::getNewInCard()
{
	return m_kHandCardData[MAX_COUNT-1-m_kWeaveCount*3];
}

void FZMJPlayer::sendOutCard(int iCard)
{
...
void FZMJPlayer::setActOutCard(int iCard,bool bDel)
{
	...
	FZMJSoundFun::playCardByGender(GetGender(),iCard);
	if (bDel)
	{
//		if (m_iIdex == 0 || m_bGameRecord || m_bMingPai)
		{
			FZMJ::CGameLogic::Instance().RemoveValueCardOne(m_kHandCardData,m_kHandCardCout,iCard);
			m_kHandCardCout --;
			FZMJ::CGameLogic::Instance().SortCardList(m_kHandCardData,m_kHandCardCout);
		}
// 		else
...
cocos2d::Node* FZMJPlayer::addHandOutCard(int iCard)
{
	int iIdex = m_iIdex;
...
void FZMJPlayer::showHandCard()
{
	showHandCard(m_kWeaveItemArray,m_kWeaveCount,m_kHandCardData,m_kHandCardCout);
	setHasCSGang();
}
void FZMJPlayer::showHandCard(CMD_WeaveItem* pWeave,int iWeaveCout,BYTE* pHandCard,int iHandCout)
{
...
void FZMJPlayer::setCardImagic(cocos2d::Node* pNode,int kValue,std::string kImagicFront)
{
	if (kValue > 0)
	{
		BYTE cbValue= FZMJ::CGameLogic::Instance().GetCardValue(kValue);
		BYTE cbColor=FZMJ::CGameLogic::Instance().GetCardColor(kValue);
...
cocos2d::Node* FZMJPlayer::getTouchCardNode(cocos2d::Vec2 kTouchPos)
{
...
BYTE FZMJPlayer::getTouchCardVlaue(cocos2d::Node* pNode)
{
	int iIdex = 0;
...
void FZMJPlayer::showHuPai(bool bZiMo)
{
	cocos2d::Node* pRootNode = WidgetFun::getChildWidget(m_pSeatNode,"CardNode1");
	for (int i = 0;i<pRootNode->getChildrenCount();i++)
	{
...
void FZMJPlayer::removeHandOutCard( BYTE cbCardData )
{
...
void FZMJPlayer::showCard()
{
	showCard(m_kHandCardData,m_kHandCardCout);
}
void FZMJPlayer::showCard(BYTE* cbCardData,BYTE cbCardCount)
{
	BYTE cbIdex = 0;
	BYTE cbChiHuCard = 0;
...
void FZMJPlayer::showJieSuanCard(cocos2d::Node* pCardNode,CMD_WeaveItem* pWeave,int iWeaveCout,BYTE* pHandCard,int iHandCout)
{
	ASSERT(pCardNode);
	pCardNode->removeAllChildren();
...
void FZMJPlayer::setChiHuCard(BYTE cbCard)
{
	m_cbChiHuCard = cbCard;
}
BYTE FZMJPlayer::getChiHuCard()
{
	return m_cbChiHuCard;
}
BYTE FZMJPlayer::getGangCard(BYTE currentCard)
{
	ASSERT(m_iIdex == 0);

	BYTE cardIndex[MAX_INDEX]={0};
	FZMJ::CGameLogic::Instance().SwitchToCardIndex(m_kHandCardData,m_kHandCardCout,cardIndex);
	if (FZMJ::CGameLogic::Instance().IsValidCard(currentCard))
	{
		bool bValue = (bool)FZMJ::CGameLogic::Instance().EstimateGangCard(cardIndex,currentCard);
		if (bValue)
		{
			return currentCard;
		}
	...
	FZMJ::tagGangCardResult kRes;
	FZMJ::CGameLogic::Instance().AnalyseGangCard(cardIndex,m_kWeaveItemArray,m_kWeaveCount,kRes);
...
void FZMJPlayer::showTingCard()
{
	...
	FZMJ::CGameLogic::Instance().SwitchToCardIndex(m_kHandCardData,m_kHandCardCout,cbCardIndex);
	FZMJ::CGameLogic::Instance().getTingCardHZ(cbCardIndex,m_kWeaveItemArray,m_kWeaveCount,kTingCardList);
...
void FZMJPlayer::showTingCard(cocos2d::Node* pNode,const std::vector<BYTE>& kTingCardList )
{
	ASSERT(pNode);
...
void FZMJPlayer::showCSGangCard(int nCard1,int nCard2,bool bOut)
{
...
void FZMJPlayer::blackAllHandCard( bool bBlack )
{
	int iIdex = 0;
	cocos2d::Node* pRootNode = WidgetFun::getChildWidget(m_pSeatNode,"CardNode1");
	for (int i = 0;i<pRootNode->getChildrenCount();i++)
