...
void FZMJGameScence::initRecord()
{
	WidgetScenceXMLparse kScence("Script/HNGameRecordChild.xml",this);

	WidgetManager::addButtonCB("Button_GameRecordPlay",this,button_selector(FZMJGameScence::Button_GameRecordPlay));
	WidgetManager::addButtonCB("Button_GameRecordPase",this,button_selector(FZMJGameScence::Button_GameRecordPase));
	WidgetManager::addButtonCB("Button_GameRecordLeft",this,button_selector(FZMJGameScence::Button_GameRecordLeft));
	WidgetManager::addButtonCB("Button_GameRecordRight",this,button_selector(FZMJGameScence::Button_GameRecordRight));
	WidgetManager::addButtonCB("Button_GameRecordExit",this,button_selector(FZMJGameScence::Button_GameRecordExit));
}
void FZMJGameScence::defaultRecordState()
{
...
void FZMJGameScence::Button_GameRecordPlay(cocos2d::Ref*,WidgetUserInfo*)
{
	...
	m_RecordTimeID = TimeManager::Instance().addCerterTimeCB(TIME_CALLBACK(FZMJGameScence::NextRecordAction,this),m_RecordTime)->iIdex;
}
void FZMJGameScence::Button_GameRecordPase(cocos2d::Ref*,WidgetUserInfo*)
{
	WidgetFun::setVisible(this,"Button_GameRecordPlay",true);
	WidgetFun::setVisible(this,"Button_GameRecordPase",false);
	TimeManager::Instance().removeByID(m_RecordTimeID);
}
void FZMJGameScence::Button_GameRecordLeft(cocos2d::Ref*,WidgetUserInfo*)
{
	m_RecordTime += 0.5f;
}
void FZMJGameScence::Button_GameRecordRight(cocos2d::Ref*,WidgetUserInfo*)
{
	m_RecordTime -= 0.5f;
...
void FZMJGameScence::Button_GameRecordExit(cocos2d::Ref*,WidgetUserInfo*)
{
	TimeManager::Instance().removeByID(m_RecordTimeID);
	ExitGame();
}
bool FZMJGameScence::StartRecord(datastream kDataStream)
{
	if (m_pGameRecord == NULL)
	{
		m_pGameRecord = new FZMJGameRecord;
		initRecord();
	}
	defaultState();
	...
	*m_pGameRecord = FZMJGameRecord();
	...
	{
		FZMJGameRecordPlayer& kRecordPlayer = m_pGameRecord->kPlayers[i];
	...
	for (int i = 0;i<(int)m_pGameRecord->kPlayers.size();i++)
	{
		...
		FZMJPlayer* pPlayer = m_pPlayer[iChairID];
		if (!pPlayer)
		{
			return false;
		}
...
void FZMJGameScence::NextRecordAction()
{
	...
	FZMJGameRecordOperateResult& kAction = m_pGameRecord->kAction[m_iActRecordIdex];
	...
	if (kAction.cbActionType == FZMJGameRecordOperateResult::TYPE_OperateResult)
	{
		FZMJPlayer* pPlayer = m_pPlayer[iChairID];
		...
		FZMJPlayer* pProvidePlayer = m_pPlayer[iProvideUser];
	...
	if (kAction.cbActionType == FZMJGameRecordOperateResult::TYPE_SendCard)
	{
		FZMJPlayer* pPlayer = m_pPlayer[iChairID];
	...
	if (kAction.cbActionType == FZMJGameRecordOperateResult::TYPE_OutCard)
	{
		FZMJPlayer* pPlayer = m_pPlayer[iChairID];
	...
	if (kAction.cbActionType == FZMJGameRecordOperateResult::TYPE_ChiHu)
	{
		FZMJPlayer* pPlayer = m_pPlayer[iChairID];
		for (int i = 0;i<MAX_PLAYER;i++)
		{
		...
		{
			FZMJPlayer* pDestPlayer = m_pPlayer[iChairID];
			pPlayer->addNewInCard(kAction.cbOperateCard);
			pPlayer->showHandCard();
...
void FZMJGameScence::onGPAccountInfoHttpIP(dword dwUserID, std::string strIP,std::string strHttp)
{
