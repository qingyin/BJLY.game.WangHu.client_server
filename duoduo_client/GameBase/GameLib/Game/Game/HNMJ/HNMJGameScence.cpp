...
void HNMJGameScence::showJieSuanInfo(CMD_S_GameEnd* pGameEnd)
{
	...
	if (pGameEnd->cbNiaoCount>0)
	{
		...
		{
			setGameEndNiaoCardAniFly(pGameEnd);
		}
	...
	for (int i=0;i<MAX_PLAYER;i++)
	{
		...
		// 加一个判断是否有胡牌的番数分，如果没有那么就表示没有玩家胡牌
		if ((pGameEnd->lGameScore[iChirID] != 0) && (pGameEnd->cbFanShu > 0))
		{
			isLiuJu = false;
		}
	}
	...
	for (int i=0;i<MAX_PLAYER;i++)
	{
		...
		//zq  修改 因为现在有抓鸟 其它玩家也可能会被扣分，分数有可能会也成负数，
		//else if (nHuScore<0 && !isZimo) 
		else if (nHuScore<0 && !isZimo)
		{
			WORD fs = pGameEnd->cbFanShu;// 胡牌番数
			WORD ns = pGameEnd->cbNiaoPick;// 鸟牌个数
			int ds = nHuScore + fs * ns;
			if (ds < 0)
	...
	if ((pGameEnd->lGameScore[nChairID]>0) && (pGameEnd->cbFanShu > 0))
	{
...
void HNMJGameScence::setGameEndNiaoCardAniFly(CMD_S_GameEnd* pGameEnd)
{
	if (!FvMask::HasAny(m_kRoomInfo.bGameRuleIdex,_MASK_(GAME_TYPE_ZZ_ZHANIAO_DIFF)))
	{
		for (int i=0;i<GAME_PLAYER;i++)
