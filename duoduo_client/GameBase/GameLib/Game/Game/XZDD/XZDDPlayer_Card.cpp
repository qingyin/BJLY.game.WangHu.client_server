...
std::vector<BYTE> XZDDPlayer::setHuanPaiSiChuan(std::vector<Card_Info> kCardInfoList)
{
	std::vector<BYTE> kCardDataList;
...
std::vector<BYTE> XZDDPlayer::setHuanPaiChengDu(std::vector<Card_Info> kCardInfoList)
{
	std::vector<BYTE> kCardDataList;
...
void XZDDPlayer::showJieSuanCard(cocos2d::Node* pCardNode,CMD_WeaveItem* pWeave,int iWeaveCout,BYTE* pHandCard,int iHandCout)
{
	ASSERT(pCardNode);
	pCardNode->removeAllChildren();
