...
//**********************************************************************************/
	...
	if (AnalyseItemArray.GetCount()>0)
	{
		...
		for (INT_PTR i=0;i<AnalyseItemArray.GetCount();i++)
		{
			...
			if(IsPengPeng(pAnalyseItem) && IsQingYiSe(cbCardIndex,WeaveItem,cbWeaveCount,cbCurrentCard))
			{
				ChiHuRight |= CHR_Qing_Da_Dui;
			}else
			{
				//碰碰和
				if( IsPengPeng(pAnalyseItem) ) 
...
bool CGameLogic::IsTingCard( const BYTE cbCardIndex[MAX_INDEX], const tagWeaveItem WeaveItem[], BYTE cbWeaveCount )
{
	...
	for( BYTE i = 0; i < MAX_INDEX-7; i++ )
	{
		...
		if( WIK_CHI_HU == AnalyseChiHuCard( cbCardIndexTemp,WeaveItem,cbWeaveCount,cbCurrentCard,chr ) )
		{
			tingCardData.push_back(i);
			std::sort(tingCardData.begin(), tingCardData.end());
			std::vector<int>::iterator iter = std::unique(tingCardData.begin(), tingCardData.end());
			tingCardData.erase(iter, tingCardData.end());
		}	

		///****************************************************************************************/
		////小胡判断  ---- 2017年3月16日21:17:33
		//if( IsXiaoHu(cbCardIndexTemp, chr) )
		//{
		//	tingCardData.push_back(i);
		//	std::sort(tingCardData.begin(), tingCardData.end());
		//	std::vector<int>::iterator iter = std::unique(tingCardData.begin(), tingCardData.end());
...
void CGameLogic::setTingCardVector(std::vector<int> m_TingCardIndex)
{
	tingCardData = m_TingCardIndex;
}
std::vector<int> CGameLogic::getTingCardVector()
{
	return tingCardData;
}